    ...
    <header className="bg-white/90 backdrop-blur-lg shadow-sm sticky top-0 z-50">
      <nav className="container mx-auto px-4 sm:px-6 py-3 flex justify-between items-center">
        ...
        <div className="hidden md:flex items-center space-x-8 text-sm font-medium text-gray-700">
            <a href="#" className="hover:text-orange-500 transition-colors">Destinos</a>
            ...
            {user ? (
                <div className="relative" ref={menuRef}>
                    ...
                    >
                        ...
                        <span className="text-sm font-medium text-gray-800 hidden lg:block">¡Hola, {user.first_name || user.username}!</span>
                        <svg className={`w-4 h-4 text-gray-600 transition-transform ${isMenuOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M19 9l-7 7-7-7"></path></svg>
                    </button>
                ...
                <>
                    <button onClick={onLoginClick} className="hidden md:block text-sm font-medium text-gray-700 hover:text-orange-500 px-4 py-2">Iniciar Sesión</button>
                    <button onClick={onRegisterClick} className="text-sm font-bold bg-orange-500 text-white px-4 py-2 rounded-full hover:bg-orange-600 shadow transition-all duration-300 transform hover:scale-105">
                        Registrarse
                    </button>
                </>
            )}
