...
function TourCard({ tour }) {
  ...
  const rating = 4.8; // Simulado
  const reviewCount = 25; // Simulado
  ...
  return (
    <div className="bg-white rounded-xl shadow-lg overflow-hidden transform hover:-translate-y-2 transition-transform duration-300 ease-in-out group flex flex-col h-full">
      <div className="relative">
        ...
        <button className="absolute bottom-3 right-3 bg-white/80 backdrop-blur-sm p-2 rounded-full text-gray-600 hover:text-red-500 transition-colors z-10"><HeartIcon className="w-5 h-5" /></button>
      </div>
      <div className="p-4 flex flex-col flex-grow">
        {tour.tags && tour.tags.length > 0 && (<span className="self-start bg-orange-500 text-white px-2 py-1 rounded-full text-xs font-semibold mb-2">{tour.tags[0].name}</span>)}
        <h3 className="text-lg font-bold text-gray-800 mb-2 group-hover:text-orange-500 transition-colors duration-200">{tour.title}</h3>
        <p className="text-sm text-gray-600 mb-3 flex-grow">{tour.description ? `${tour.description.substring(0, 70)}...` : ''}</p>
