...
function FeaturedDestinations() {
  ...
  const handleDestinationClick = (destination) => {
    navigate(`/destinos?destination=${encodeURIComponent(destination.state || destination.name)}`);
  };
  ...
  if (destinations.length === 0) {
    return (
      <Section title="Destinos Destacados">
        <div className="text-center py-12">
          <p className="text-gray-500">No hay destinos disponibles en este momento</p>
        </div>
      </Section>
    );
  }
